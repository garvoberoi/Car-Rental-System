<?php
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
        ...
        if (empty($_POST['cus_email'])) {
            ...
            $email = test_input($_POST["cus_email"]);
        }
        if (empty($_POST['cus_password'])) {
            $rErr = true;
        } else {
            $pass = test_input($_POST["cus_password"]);
        }
        ...
        if($num == 1){
            ...
            if(!$rErr){
                $sql = "INSERT INTO customers (name, email, password, date) VALUES ('$name', '$email', '$pass', current_timestamp())";
                $result = mysqli_query($conn, $sql);
                if($result){
                    session_start();
    ...
    <div>
        <div class="log-cont">
            ...
            <form method="POST" class=form-login>
                <input type="text" placeholder="Username" name="cus_name" class="input-login">
                <input type="text" placeholder="Email" name="cus_email" class="input-login">
                <input type="password" placeholder="Password" name="cus_password" class="input-login">
